...
<script>
    $(function(){
        ...
        $(".file-upload").each(function () {
            ...
            self.fileupload({
                url: '/<?php echo $controller ?>/saveFile/',
                dataType: 'json',
                autoUpload: true,
                add: function (e, data) {
                    ...
                    if (data.result.state == 0) {
                        ...
                        fileNameTd.html($("<a target='_blank' class='btn btn-primary btn-xs'></a>").attr("href","/<?php echo $controller ?>/getFile/?id="+data.result.data+"&fileName="+fileNameTd.attr("name")).html(fileNameTd.attr("name")));
                        btnText.html("重新上传");
